...
String ver= "1.1";    // версия прошивки

int SensorValue, PressureValue;
int engine_animation = 0;
int engine_animation_enable = 0;
int n_menu = 0;
byte menu = 0;
...
void Switcher() {
   if ( SensorValue <= PinUp )  {
     digitalWrite(SwitcherPin, HIGH);
     engine_animation_enable = 1;
   }
   if ( SensorValue >= PinDown ) {
     digitalWrite(SwitcherPin, LOW);
     engine_animation_enable = 0;
   }
}
...
void engine_anim() { // анимация на включение реле
  engine_animation = engine_animation + 3;
  if ( engine_animation > 15 ) engine_animation = 0;
  myGLCD.print(">>", engine_animation, 10);
  myGLCD.print("<<", 115-engine_animation, 10);
}
...
void lcd_main() { // основной экран
  ...
  myGLCD.print("Volt/100", RIGHT, 41);
  if ( engine_animation_enable == 1 ) engine_anim();
  myGLCD.update();
}
...
void EEpromRead() { // чтение настроек из памяти
